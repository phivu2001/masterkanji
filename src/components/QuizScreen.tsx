...
export function QuizScreen({ pool, level, mode, requestedQuestionCount, title, settings, onExit, onComplete }: Props) {
  ...
  const quizTitle = title ?? (mode === 'exam' ? `Thi thử ${level}` : `Quiz ${level}`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-4 sm:p-8">
      <div className="max-w-3xl mx-auto">
        <header className="flex items-center justify-between mb-8"><button onClick={onExit} aria-label="Thoát Quiz" className="w-11 h-11 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700"><i className="fas fa-xmark"></i></button><div className="text-center"><div className="font-black">{quizTitle}</div><div className="text-xs text-slate-400">Câu {index + 1}/{questions.length}</div></div>{mode === 'exam' ? <div className={`font-mono font-black px-3 py-2 rounded-lg ${secondsLeft < 60 ? 'bg-red-500 text-white' : 'bg-white dark:bg-slate-800'}`}>{minutes}:{seconds}</div> : <div className="w-11" />}</header>
