...
export function VocabularyTypingScreen({ pool, level, title, settings, onExit, onComplete }: Props) {
  ...
  const checkAnswer = () => {
    ...
    const correct = analyzeJapaneseAnswer(committed, acceptedAnswers(current)).correct;
    setValue(committed);
    setSubmitted(committed);
    setResult(correct ? 'correct' : 'incorrect');
    if (correct) setAnswers((items) => [...items, { wordId: current.id, correct: !hadMistake }]);
    else setHadMistake(true);
  };
  ...
  const reveal = () => {
    ...
    setAnswers((items) => [...items, { wordId: current.id, correct: false }]);
  };

  const next = () => {
    if (result !== 'correct' && !revealAnswer) return;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-4 sm:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <section className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-7 sm:p-10 shadow-sm">
          ...
          <form onSubmit={(event) => { event.preventDefault(); if (result === 'idle') checkAnswer(); else if (result === 'correct' || revealAnswer) next(); }}>
            <input autoFocus value={value} disabled={result === 'correct' || revealAnswer || result === 'incorrect'} onChange={(event) => setValue(convertRomajiInput(event.target.value))} autoComplete="off" spellCheck={false} lang="ja" inputMode="text" placeholder="Kanji, Kana hoặc Romaji" className="w-full rounded-2xl border-2 border-blue-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 px-5 py-4 text-2xl font-japanese outline-none focus:border-blue-500 disabled:opacity-80" />
            ...
            {result === 'incorrect' && <JapaneseAnswerFeedback input={submitted} acceptedAnswers={acceptedAnswers(current)} revealAnswer={revealAnswer} onRetry={retry} onReveal={reveal} />}
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              ...
              <button disabled={result === 'incorrect' && !revealAnswer} className="flex-1 px-5 py-3 rounded-xl bg-blue-500 text-white disabled:opacity-40 font-black">{result === 'idle' ? 'Kiểm tra' : result === 'incorrect' && !revealAnswer ? 'Thử lại hoặc xem đáp án' : index === questions.length - 1 ? 'Xem kết quả' : 'Từ tiếp theo'}<i className="fas fa-arrow-right ml-2"></i></button>
            </div>
          </form>
        </section>
      </div>
